...
// オブジェクトを強参照として取得
void
ObjectInfo::getStrong(const ObjectInfo &src)
{
	clear();
	HSQUIRRELVM gv = getGlobalVM();
	src.push(gv);
...
void
ObjectInfo::throwError(HSQUIRRELVM v) const
{
	sq_throwobj(v, obj);
}
...
/// 配列として初期化
void
ObjectInfo::initArray(SQSize size)
{
	clear();
	HSQUIRRELVM gv = getGlobalVM();
	sq_newarray(gv, (SQInteger)size);
	sq_getstackobj(gv, -1, &obj);
	sq_addref(gv, &obj);
	sq_pop(gv, 1);
}
...
void pushValue(HSQUIRRELVM v, SQLong value) { sq_pushlong(v,value); }
...
void pushValue(HSQUIRRELVM v, const sqstring &value) { sq_pushstring(v,value.c_str(),(SQInteger)value.length()); }
...
// 値の取得
#if 0
SQRESULT getValue(HSQUIRRELVM v, bool *value, int idx) { SQBool b;SQRESULT ret = sq_getbool(v, (SQInteger)idx, &b); *value = b != SQFalse; return ret; }
SQRESULT getValue(HSQUIRRELVM v, SQInteger *value, int idx) { return sq_getinteger(v, (SQInteger)idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQLong *value, int idx) { return sq_getlong(v, (SQInteger)idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQFloat *value, int idx) { return sq_getfloat(v, (SQInteger)idx, value); }
SQRESULT getValue(HSQUIRRELVM v, const SQChar **value, int idx) { return sq_getstring(v, (SQInteger)idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQUserPointer *value, int idx) { return sq_getuserpointer(v, (SQInteger)idx, value); }
SQRESULT getValue(HSQUIRRELVM v, ObjectInfo *value, int idx) { value->getStack(v, (SQInteger)idx); return SQ_OK; }
SQRESULT getValue(HSQUIRRELVM v, sqstring *value, int idx) {const SQChar *str; SQRESULT ret; if (SQ_SUCCEEDED((ret = sq_getstring(v, (SQInteger)idx, &str)))) { *value = str;} return ret;}
#endif
SQRESULT getValue(HSQUIRRELVM v, bool *value, SQInteger idx) { SQBool b;SQRESULT ret = sq_getbool(v, idx, &b); *value = b != SQFalse; return ret; }
SQRESULT getValue(HSQUIRRELVM v, SQInteger *value, SQInteger idx) { return sq_getinteger(v, idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQLong *value, SQInteger idx) { return sq_getlong(v, idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQFloat *value, SQInteger idx) { return sq_getfloat(v, idx, value); }
SQRESULT getValue(HSQUIRRELVM v, const SQChar **value, SQInteger idx) { return sq_getstring(v, idx, value); }
SQRESULT getValue(HSQUIRRELVM v, SQUserPointer *value, SQInteger idx) { return sq_getuserpointer(v, idx, value); }
SQRESULT getValue(HSQUIRRELVM v, ObjectInfo *value, SQInteger idx) { value->getStack(v, idx); return SQ_OK; }
SQRESULT getValue(HSQUIRRELVM v, sqstring *value, SQInteger idx) {const SQChar *str; SQRESULT ret; if (SQ_SUCCEEDED((ret = sq_getstring(v, idx, &str)))) { *value = str;} return ret;}

// 値の強制初期化
void clearValue(bool *value) { *value = false; }
void clearValue(SQInteger *value) { *value = 0; }
void clearValue(SQLong *value) { *value = 0; }
...
SQRESULT getResultValue(HSQUIRRELVM v, SQLong *value) { return sq_getlong(v, -1, value); }
