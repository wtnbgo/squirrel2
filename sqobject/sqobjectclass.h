...
public:
	 ...
	 */
	Object(HSQUIRRELVM v, SQInteger delegateIdx=2);
	 ...
	 */
	void initSelf(HSQUIRRELVM v, SQInteger idx=1);
...
protected:
	 ...
	 * @param p2 引数3
	 */
	template<typename R, typename T1, typename T2, typename T3> SQRESULT callEventResult(R* r, const SQChar *eventName, T1 p1, T2 p2, T3 p3) {
		return self.callMethodResult(r, eventName, p1, p2, p3);
	}
...
// 値の取得
template<typename T>
SQRESULT getValue(HSQUIRRELVM v, T **value, SQInteger idx) {
...
public:
  // コンストラクタ
  StackValue(HSQUIRRELVM v, SQInteger idx) : v(v), idx(idx) {};
  StackValue(HSQUIRRELVM v, SQUnsignedInteger idx) : v(v), idx(idx) {};
  ...
  {
	T value;
	SQRESULT ret = getValue(v, &value, idx);
	if (SQ_FAILED(ret)) {
	  clearValue(&value);
	}
	return value;
...
public:
	 ...
	 * @return 引数の型
	 */
	SQObjectType getType(SQInteger n) const {
		if (n < argc) {
	 ...
	 * @return 引数の型
	 */
	ObjectInfo getArg(SQInteger n) const {
		ObjectInfo ret;
		if (n < argc) {
			ret.getStack(vm, n+2);
		}
	 ...
	 * @return 引数オブジェクト
	 */
	ObjectInfo operator[](SQInteger n) const {
		return getArg(n);
	}
