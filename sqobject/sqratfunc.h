...
// 非継承クラス・デフォルトコンストラクタ
#define SQCLASS(Target, Name)\
  Sqrat::Class<Target, Sqrat::DefaultAllocator<Target> > cls(sqobject::getGlobalVM());\
  Sqrat::RootTable(sqobject::getGlobalVM()).Bind(Name, cls);

// 非継承クラス・引数ありコンストラクタ
#define SQCLASS_ARGSCONSTRUCTOR(Target, Name, Args)                               \
  Sqrat::Class<Target, Sqrat::ArgsConstructor<Target, void(Target::*) Args> > \
      cls(sqobject::getGlobalVM());                                           \
  Sqrat::RootTable(sqobject::getGlobalVM()).Bind(_SC(#Name), cls)
...
// 非継承クラス・ファクトリ形式
#define SQCLASS_FACTORY(Target, Name)\
  Sqrat::Class<Target, Sqrat::Factory<Target> > cls(sqobject::getGlobalVM());\
  Sqrat::RootTable(sqobject::getGlobalVM()).Bind(Name, cls);
