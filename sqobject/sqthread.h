...
protected:
	...
	SQInteger _priority; ///< このスレッドの実行優先度
...
public:
	 ...
	 * result取得
	 */
	SQRESULT getResult(HSQUIRRELVM v);
	 ...
	 */
	SQRESULT getstackinfos(HSQUIRRELVM v);


	/**
	 * @return スレッド実行優先度
	 */
	SQInteger getPriority() const {
		return _priority;
	}
	 ...
	 */
	void setPriority(SQInteger priority) {
		_priority = priority;
	}

	SQBool getStepMode() { return _stepMode; }
	void setStepMode(SQBool mode) { _stepMode = mode; }
	
	SQRESULT step(HSQUIRRELVM v);
...
protected:
	...
	// オブジェクトソート処理用
	typedef Thread *OBJP;
	// 順序比較関数: priority でソートする
	struct lessObj {
		bool operator() (const OBJP &obj1, const OBJP &obj2)  const {
			return obj1->_priority < obj2->_priority;
		}
	};
