...
static SQRESULT Thread_getPriority(HSQUIRRELVM v)
{
	Thread *instance = SQClassType<Thread>::getInstance(v);
	if (instance) {
		sq_pushinteger(v,instance->getPriority());
		return 1;
	}
	return ERROR_BADINSTANCE(v);
}

static SQRESULT Thread_setPriority(HSQUIRRELVM v)
{
	Thread *instance = SQClassType<Thread>::getInstance(v);
	if (instance) {
		SQInteger priority = DEFAULT_PRIORITY;
		if (sq_gettop(v) >= 2) {
			sq_getinteger(v, 2, &priority);
		}
...
static SQRESULT Thread_getStepMode(HSQUIRRELVM v)
{
	Thread *instance = SQClassType<Thread>::getInstance(v);
	if (instance) {
		sq_pushbool(v,instance->getStepMode());
		return 1;
	}
	return ERROR_BADINSTANCE(v);
}

static SQRESULT Thread_setStepMode(HSQUIRRELVM v)
{
	Thread *instance = SQClassType<Thread>::getInstance(v);
	if (instance) {
		SQBool mode = false;
...
Thread::registerClass()
{
	...
	SQNFUNC(Thread,getResult);
	SQNFUNC(Thread,wait);
	SQNFUNC(Thread,cancelWait);
	SQNFUNC(Thread,getPriority);
	SQNFUNC(Thread,setPriority);
	SQNFUNC(Thread,setStepMode);
	SQNFUNC(Thread,getStepMode);
	SQNFUNC(Thread,step);
	SQNFUNC(Thread,getstackinfos); // debug
#endif
};
