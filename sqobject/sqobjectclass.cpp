...
{
	SQInteger len = (SQInteger)(sizeof(SQChar) * (scstrlen(name) + 4));
 ...
 * コンストラクタ
 */
Object::Object(HSQUIRRELVM v, SQInteger delegateIdx)
{
 ...
 */
void
Object::initSelf(HSQUIRRELVM v, SQInteger idx)
{
	if (sq_gettop(v) >= idx) {
	...
	if (name && *name) {
		...
		if (delegate.isDelegate()) {
			...
			if (SQ_SUCCEEDED(sq_get(v,-2))) {
				// メソッドの場合は束縛処理
				if (isClosure(sq_gettype(v,-1))) {
					if (!sq_hasenv(v, -1) && delegate.isBindDelegate()) {
						delegate.push(v);
