...
#endif
	/// コンストラクタ
	SQOBJFile(const SQChar *filename, bool binary) : file(NULL), buffer(NULL), size(0), binary(binary) {
...
{
	return (void*) new SQOBJFile(filename, binary);
}
...
{
	SQOBJFile *file = (SQOBJFile*)handler;
	if (file) {
		if (file->check()) {
