...
// コンストラクタ
Thread::Thread() : _currentTick(0), _priority(THREAD_DEFAULT_PRIORITY), _fileHandler(NULL), _waitTimeout(-1), _status(THREAD_NONE), _result(0), _stepMode(false)
{
	_waitList.initArray();
}

// コンストラクタ
Thread::Thread(HSQUIRRELVM v) : Object(v), _currentTick(0), _priority(THREAD_DEFAULT_PRIORITY), _fileHandler(NULL), _waitTimeout(-1), _status(THREAD_NONE), _result(0), _stepMode(false)
{
	_waitList.initArray();
	// 実行
	if (sq_gettop(v) >= 3) {
 ...
 */
SQRESULT
Thread::getResult(HSQUIRRELVM v)
{
	sq_pushinteger(v, _result);
	return 1;
}
...
Thread::getStatus()
{
	return (SQInteger)(isWait() ? THREAD_WAIT : _status);
}

/**
 * 動作スレッドを対象としたgetstackinfos()
 * @return getstackinfos()の戻り値(stack情報辞書)
 */
SQRESULT
Thread::getstackinfos(HSQUIRRELVM v)
{
	// 中身は基本的にbase_getstackinfos()と同じ。
	// ただしcallstackのみで、スタック内容localは(メンドいので)取得不可。
	SQInteger level;
	SQStackInfos si;
	SQInteger seq = 0;
	const SQChar *name = NULL;
	sq_getinteger(v, -1, &level);
	if (SQ_SUCCEEDED(sq_stackinfos(_thread, level, &si)))
	{
...
/*
		...
		sq_newtable(v);
		seq=0;
		while ((name = sq_getlocal(_thread, level, seq))) {
...
Thread::_main(long diff)
{
	...
	if (_status == THREAD_LOADING_FILE) {
		...
		if (sqobjCheckFile(_fileHandler, &dataAddr, &dataSize)) {
			_init();
			_result = sqstd_loadmemory(_thread, dataAddr, dataSize, _scriptName.getString(), SQTrue);
			sqobjCloseFile(_fileHandler);
			_fileHandler = NULL;
			if (SQ_SUCCEEDED(_result)) {
				_status = THREAD_RUN;
			} else {
				// exit相当
...
Thread::_wakeup()
{
	...
	if (!isWait() && _status == THREAD_RUN) {
		if (sq_getvmstate(_thread) == SQ_VMSTATE_SUSPENDED) {
			...
			_waitResult.clear();
			_result = sq_wakeupvm(_thread, SQTrue, SQTrue, SQTrue, SQFalse);
		} else {
			sq_pushroottable(_thread);
			SQInteger n = _args.pushArray(_thread) + 1;
			_args.clear();
			_result = sq_call(_thread, n, SQTrue, SQTrue);
		}
		if (SQ_FAILED(_result)) {
			// スレッドがエラー終了
			sq_getlasterror(_thread);
			_exitCode.getStack(_thread, -1);
			sq_pop(_thread, 1);
...
SQRESULT
Thread::step(HSQUIRRELVM v)
{
	if (_stepMode){
		if (!isWait() && _status == THREAD_RUN) {
			_wakeup();
			if (_status == THREAD_NONE) { // 終了した
				if (SQ_FAILED(_result)) {
					_exitCode.throwError(v);
					return SQ_ERROR;
				} else {
					_exitCode.push(v);
					return 1;
...
Thread::printError()
{
	SQPRINTFUNCTION print = sq_getprinterrfunc(_thread);
	if (print) {
		sq_getlasterror(_thread);
		const SQChar *err;
...
Thread::main(ThreadCallback *onThreadDone, void *userData)
{
	SQInteger i=0;
	SQInteger max = threadList->len();
	// 現在有効ナスレッドを優先度順にソートして実行
	all.clear();
	for (i=0;i<max;i++) {
		ObjectInfo thObj = threadList->get(i);
		Thread *th = thObj;
		if (th && th->_status != THREAD_NONE) {
			all.insert(th);
		}
	}
	...
	// 終了済スレッドを破棄
	// 処理してる最中に threadList に追加される場合があるので値をとりなおす
	i=0;
	max = threadList->len();
	while (i < max) {
		ObjectInfo thObj = threadList->get(i);
...
{
	sq_pushinteger(v, (SQInteger)currentTick);
	return 1;
}
...
{
	sq_pushinteger(v, (SQInteger)diffTick);
	return 1;
}
...
#define REGISTERENUM(name, value) \
	...
	REGISTERENUM(THREAD_DEFAULT_PRIORITY, THREAD_DEFAULT_PRIORITY);
	sq_pop(v, 1); // root
