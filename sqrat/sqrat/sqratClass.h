...
namespace Sqrat {
	...
	*/
	...
	public:
		...
		*/
		/// Constructor
		Class(HSQUIRRELVM v = DefaultVM::Get(), bool createClass = true) : Object(v, false), inited(false) {
			if(createClass && !ClassType<C>::Initialized()) {
				HSQOBJECT& classObj = ClassType<C>::ClassObject();
				sq_resetobject(&classObj);

				sq_newclass(vm, false);
				sq_settypetag(vm, -1, (SQUserPointer)&classObj);
	...
	public:
		...
		*/
		...
		Class& StaticFunc(const SQChar* name, F method) {
			BindFunc(name, &method, sizeof(method), SqGlobalFunc(method), true);
			return *this;
		}

		//template<class F>
		Class& SquirrelFunc(const SQChar* name, SQFUNCTION func, bool staticFunc=false) {
			sq_pushobject(vm, ClassType<C>::ClassObject());
			sq_pushstring(vm, name, -1);
			sq_newclosure(vm, func, 0);
			sq_newslot(vm, -3, staticFunc);
		...
		Class& VarArgFunc(const SQChar* name, F method) {
			BindFunc(name, &method, sizeof(method), SqVarArgMemberFunc(method));
			return *this;
		}

		template<class F>
		Class& BridgeFunc(const SQChar* name, F method) {
			BindFunc(name, &method, sizeof(method), SqBridgeFunc<C>(method));
			return *this;
		}

		template<class F>
		Class& ProxyFunc(const SQChar* name, F method) {
			BindFunc(name, &method, sizeof(method), SqProxyFunc<C>(method));
			return *this;
		}

		template<class F>
		Class& CastFunc(const SQChar* name, F method) {
			BindFunc(name, &method, sizeof(method), SqCastMemberFunc<C>(method));
			return *this;
		}
